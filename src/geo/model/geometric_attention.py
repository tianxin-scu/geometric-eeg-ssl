"""Geometric attention: 10-D descriptor, per-electrode tokens (no CLS pool).

Design notes:

1. **10-D descriptor.** A minimal ``g_ij = [||p_i - p_j||, p_i - p_j]`` (R^4)
   is translation-invariant by construction, and a ``b`` trained on it
   collapses to a per-head linear ramp in distance. The descriptor used here,

       g_ij = [p_i, p_j, p_i - p_j, ||p_i - p_j||]  in R^{3+3+3+1} = R^10

   is a strict superset: the MLP can recover the distance-only behavior by
   ...
   Coordinates must be in a shared frame (`src/geo/preprocess.py`).

2. **No CLS pool.** The spatial encoder returns the full per-electrode token
   sequence ``(B, M, d)`` so the downstream loss can align per-channel.

The g1/g2/g3 modes, geometry-table precomputation, and pre-norm blocks are
standard. The descriptor builder is in this file; the spatial encoder is the
``cls=False`` variant.
"""
...
#              only -- drops displacement + distance, which the trained b
#              tends to collapse onto. Ablation: does absolute position alone
#              carry the cross-montage signal?)
_DESCRIPTOR_DIMS = {"full": 10, "pos_only": 6}

# Width of the full descriptor (kept for backward-compatible imports).
G_DIM = _DESCRIPTOR_DIMS["full"]
...
def build_geometric_descriptor(
    ...
    displacement + distance terms.

    Args:
        ch_pos: (M, 3) coordinates in a shared frame
            (see `src/geo/preprocess.py`). Either numpy or torch is fine.
...
# ---------------------------------------------------------------------------
# Multi-head attention with geometric conditioning (10-D MLP inputs)
# ---------------------------------------------------------------------------
...
class GeometricMultiheadAttention(nn.Module):
    ...
    The geometric MLPs accept a 10-D descriptor.
    """
    ...
    def forward(
        ...
        """
        Args:
            x: (B, M, d_model). No CLS token; tokens are electrodes.
...
class GeometricEncoderBlock(nn.Module):
    """Pre-norm Transformer block with geometric attention."""
...
class GeometricSpatialEncoder(nn.Module):
    ...
    def precompute_geom_tables(self, ch_pos: torch.Tensor):
        ...
        g = build_geometric_descriptor(ch_pos, self.descriptor)  # (M, M, D)
...
if __name__ == "__main__":
    ...
    # Apply a fixed-scale normalization (not the real REF_* constants,
    ...
    g = build_geometric_descriptor(ch_pos)
    ...
    print("geometric_attention.py smoke OK")
