"""Fixed-scale coordinate normalization for v2.

v1's `get_channel_positions` normalizes each montage by its own centroid and
max-norm, which makes the same physical electrode land at different `(x,y,z)`
across datasets (e.g. Cz lives at different coordinates in PhysioNet vs
BCIC-2B vs Sleep-EDFx). That's fine when the geometric MLP consumes only
translation-invariant features like distance + displacement, but v2.0 feeds
absolute `p_i, p_j` into `b`, and absolute coordinates only carry signal
across montages if they share a frame.
...
subset of electrodes:
    ...
    pos_v2 = (pos_raw - REF_CENTROID) / REF_SCALE
...
This is a Tier-1 preprocessing change (signed off in v2 scoping). It does
NOT touch the signal pipeline -- bandpass, resample, epoching, amplitude
reject, normalize_epochs are unchanged. Only coordinates change. v1's
cached signal data is therefore reusable; we just recompute `ch_pos` here
from `ch_names`.
"""
...
# (343 electrodes). Computed once via the diagnostic in the v2 scoping
# session; pinning them here keeps v2 reproducible without re-instantiating
...
# Sleep-EDFx bipolar channel names (short form, as used by the v1 loader)
...
if __name__ == "__main__":
    # Spot-check against the diagnostic numbers from the v2 scoping session.
    phys_names = [
