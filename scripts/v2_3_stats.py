"""v2.3 significance test: single-montage zero-shot, geom/pos_only vs chind.
...
Usage:
    python scripts/v2_3_stats.py \\
        --geom      runs/pretrain/v2_3_geometric/probe_sleep_edfx_within_subject_night.json \\
        --pos-only  runs/pretrain/v2_3_pos_only/probe_sleep_edfx_within_subject_night.json \\
        --chind     runs/pretrain/v2_3_chind/probe_sleep_edfx_within_subject_night.json
...
def main(argv=None):
    p = argparse.ArgumentParser(description="v2.3 paired significance test.")
    ...
    print(f"\n=== v2.3 paired comparison ({Path(args.geom).name}) ===\n")
