"""Aggregate v2 probe JSONs into the headline cross-montage BAC table.

Reads the per-(variant, held-out) probe outputs written by
`scripts/v2_probe.py` and emits the 3-row x 2-column (geometric, chind)
table of zero-shot BAC. Codex column is left out until that variant lands.

Usage:
    python scripts/v2_results.py
    python scripts/v2_results.py --eval-dir runs/eval/v2 --out-dir results/v2
"""
...
def _render_text(cells: dict, cols: list[str]) -> str:
    lines = []
    lines.append("v2 cross-montage zero-shot BAC (mean +/- std over LOSO/night folds)")
    lines.append("=" * 70)
...
def main(argv=None):
    ...
    p.add_argument("--eval-dir", default="runs/eval/v2")
