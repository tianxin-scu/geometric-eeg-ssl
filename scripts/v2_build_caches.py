"""Build v2 signal caches (9 subjects/dataset) on the local machine.

Mirrors `notebooks/colab_v2_pretrain.ipynb` section 4b for local execution.
Downloads raw data via MNE/MOABB if not present, then writes the v1 signal
cache (which v2 reuses unchanged) into EEG_CACHE_DIR.
...
Usage:
    python scripts/v2_build_caches.py
    python scripts/v2_build_caches.py --n-subjects 9 --cache-dir D:/eeg_cache
"""
