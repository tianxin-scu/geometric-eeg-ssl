"""Cross-montage pretraining (leave-one-dataset-out).

Mixed-corpus pretraining for the backbone. Specify two of three datasets
...
Usage (no-sleep leave-one-out):
    python scripts/pretrain.py \\
        ...
        --ckpt-dir runs/pretrain/no_sleep

Usage (resume):
    python scripts/pretrain.py ... --resume latest

Pipeline notes:
- Loads two datasets and mixes them via MixedCorpus.
- Recomputes geometry tables PER DATASET PER STEP (different montage).
- Uses Backbone + PretrainModel (per-electrode tokens, per-channel
  alignment + per-electrode recon).
- Uses src/geo/preprocess.ch_pos_from_names for fixed-scale coordinates.

The signal cache is reused (preprocessing knobs unchanged). Set
...
from src.geo.datasets.mixed_corpus import MixedCorpus
from src.geo.losses import PretrainModel
...
def _parse_args(argv=None):
    p = argparse.ArgumentParser(description="Cross-montage pretraining.")
    ...
    p.add_argument(
        ...
        help="Comma-separated names from {physionet_mi, bcic_2b, sleep_edfx}. "
             ...
             "both held out; this is the single-montage design).",
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        help="Cap subjects per dataset (subject-level balancing). "
    ...
    p.add_argument(
        ...
        help="Spatial encoder variant. 'geometric' = g3 + 10-D descriptor "
             ...
             "'codex' variant is deferred.",
    )
    p.add_argument(
        "--ckpt-dir", default=None,
        help="Default: runs/pretrain/<timestamp>.",
    )
...
def _load_dataset(
    ...
    """Run the dataset loader (cached signals), recompute ch_pos.

    Returns (X, ch_pos, ch_names) -- ch_pos is the fixed-scale variant
    derived from ch_names, NOT what the loader put in its cache.
    ...
    if name == "physionet_mi":
        ...
        if n_subjects is not None:
            subjects = subjects[:n_subjects]
        X, _y, _cp, ch_names, _night = SleepEDFx(
            subjects=subjects, cfg=cfg, mode="pretrain", verbose=True
        ).load()
    else:
...
def _build_model(cfg: Config, variant: str) -> PretrainModel:
    """Build the PretrainModel for one of the variants.
    ...
    if abl.use_codex:
        raise ValueError(
            "No codex baseline yet (deferred); use_codex must be False."
        )
...
def main(argv=None):
    ...
    # A single pretrain dataset is allowed: the MixedCorpus round-robin
    # degenerates to that one dataset, and `held_out` becomes the OTHER two,
    # both probed zero-shot.
    ...
    if args.ckpt_dir is None:
        ...
        args.ckpt_dir = f"runs/pretrain/{args.variant}_{ts}"
